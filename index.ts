...
window.addEventListener("load", async () => {
    ...
    let leftPlayer: number = 0;
    let rightPlayer: number = 0;
    ...
    // Controls the speed of the movement of the paddle (number of pixels per interval)
    const speed = 1;
    ...
    function animateBall(currentBallPosition: Point, targetBallPosition: Point): Promise<{touchPosition: Point, touchDirection: Direction}> {
      ...
      return new Promise<{touchPosition: Point, touchDirection: Direction}>(res => {
        ...
        const interval = setInterval(() => {
          ...
          let lostLeft:boolean = false;
          if ((animatedPosition.x - ballHalfSize.width) < 0) { touchDirection = Direction.left; lostLeft=true;}
          ...
          if ((animatedPosition.x - ballHalfSize.width) < paddle.clientWidth+50 && animatedPosition.y > currentPaddlePosition && animatedPosition.y < currentPaddlePosition+paddleHeight){ touchDirection = Direction.left; }

          if (lostLeft === true){
            p.textContent = `${leftPlayer}:${++rightPlayer}`;
          }
